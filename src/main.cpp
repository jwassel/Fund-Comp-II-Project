...
//returns 1 if succesfully loaded a game, 0 otherwise
int load_previous_game(Dome &dome){
...
infile.open(input.c_str());
	...
	while(!infile.good()){
	cout<<"This file does not exist, try again! Type q play without a saved game"<<endl;
	cin>>input;
	if(input=="q")
	{
		cout<<"Go back to the game!"<<endl;
		infile.close();
		return 0; 
	}
	...
	string header;
	infile>>header;
	while(header != "PokedomeSavedGame")
	{
		cout<<"Not a valid Pokedome saved game. Please try again! Type q to play without saved game"<<endl;
		cin>>input;
		if(input=="q")
		{
			cout<<"Go back to the game!"<<endl;
			...
			if (event.type == SDL_MOUSEBUTTONDOWN)
			{
				...
				if (event.button.button == SDL_BUTTON_LEFT)
				{
					...
					if(loadButton.isClicked(x,y)){
						...
						int loaded = load_previous_game(dome);
						if(loaded) play=true;
