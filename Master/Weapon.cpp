...
#include <iostream>
using namespace std;
Weapon::Weapon (string filename, string explosionName, int Price,
		int AmmoPrice, int Damage, int FireRate, int x, int y,
		int expsize, int max, int current, int maxclip,
		int currentclip)
{
...
void Weapon::handle_events (SDL_Event event, vector < Enemy * >enemies,
		       SDL_Surface * screen, int &score, int &money)
{
      ...
      if (currentClipAmmo > 0)
	{
	  ...
	  if (event.type == SDL_MOUSEBUTTONDOWN)
	      ...
	      if (event.button.button == SDL_BUTTON_LEFT)
		{
		  ...
		  x = event.button.x;
		  y = event.button.y;
		  showExplosion (x - getSize () / 2, y - getSize () / 2,
				 screen);
...
void Weapon::apply_surface (int x, int y, SDL_Surface * source,
...
SDL_Surface * Weapon::load_image (std::string filename)
{
  ...
  if (loadedImage != NULL)
      ...
      if (optimizedImage != NULL)
	{
	  //Map the color key
	  Uint32 colorkey =
	    SDL_MapRGB (optimizedImage->format, 0xec, 0x26, 0x26);
