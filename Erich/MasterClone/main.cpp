...
//show the enemies  
            for (int j = 0; j < enemies.size (); j++)
	{ 
	if (enemies[j]->isPika() && dome.isCollidingWithEnemy(enemies[j]->getX(), enemies[j]->getY(), enemies[j]->getWidth(), enemies[j]->getHeight(), enemies[j]->getXVel()))
	  enemies[j]-> showAttack(screen, count);
	else
	  enemies[j]->show (screen, count);
	}
